...
### 
# Installation and autoconfigure script for openSUSE systems and dnscrypt.
#
...
# Modified for openSUSE by Lukmanul Hakim: https://luqmanulhackim.wordpress.com
#
###
...
config_resolv() {
	...
	echo "# Generated by DNSCrypt Auto Installer" > /etc/resolv.conf
	echo "search suse" >> /etc/resolv.conf
...
config_del() {
	...
	/etc/init.d/dnscrypt-proxy stop
	chkconfig --del dnscrypt-proxy
	chown dnscrypt /etc/dnscrypt/run
	rm -rfd /var/mail/dnscrypt
	touch /var/mail/dnscrypt
	chown dnscrypt /var/mail/dnscrypt
	userdel -r dnscrypt
...
if [ "$DNSCRYPTINST" == "true" ]; then
	if [ "$DNSCRYPTCONF" == "true" ]; then
		...
		echo "    ./dnscrypt-autoinstall-openSUSE.sh forcedel"
		echo ""
		echo "Quitting."
		exit 1
	fi
	...
	if nc -z -w1 127.0.0.1 53; then
		...
		sudo bash <<EOF
		zypper ref
		zypper in -n make automake gcc gcc-c++ libtool ca-certificates curl
EOF
		[ ! -d "$TMPDIR" ] && mkdir "$TMPDIR"
		pushd "$TMPDIR"
		
		# openSUSE include libsodium
		zypper in -n libsodium-devel && LSODIUMINST=true
		...
		if [ "$LSODIUMINST" == "false" ]; then
			...
			# openSUSE does not include /usr/local/lib for linking
